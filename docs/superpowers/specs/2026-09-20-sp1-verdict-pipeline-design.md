...
J1은 사용자 결정, J2~J8은 이 스펙의 설계 결정이다(J8은 구현 계획 작성 중 추가). 검토 때 특히 봐 줄 것.
...
| J5 | **공식 등급 = 주간 스냅샷**(`user_grade_current`). 제보권 한도와 앱의 현재 등급이 같은 값을 쓰고, 다음 등급까지의 진행 상황만 실시간으로 계산한다 | 01 §3.3 "표시와 집행을 같은 소스로". `MeService`는 "스냅샷은 최대 일주일 묵는다"는 이유로 실시간 등급을 택했지만(클래스 주석), SP1로 TI가 실제로 움직이면 실시간 등급이 주중에 오르내려 "L1인데 제보권은 L0"이 생기고, 강등 규칙(01 §6.1) 없이 화면상 강등이 일어난다. 그 선택을 이 스펙이 뒤집는다 |
| J8 | 강등 규칙(Phase 2) 전까지 `grade_recalc`는 **등급을 내리지 않는다** — 새 스냅샷 등급 = max(직전 스냅샷, 이번 평가) | 01 §6 "강등 규칙 구현 전까지 강등은 발생하지 않는다". 지금은 아무도 승급하지 못해 드러나지 않았을 뿐, SP1 뒤에는 TI 하락만으로 다음 주에 등급이 내려간다(4주 연속·1단계 규칙 없이) |
...
- 새 스냅샷 등급 = max(직전 스냅샷 등급, `GradePolicy.evaluate` 결과)(J8). TI·AS·판정완료 값은 이번 평가값 그대로 기록한다 — 등급만 유지.
...
| 16 | 강등 없음: L1 스냅샷 유저의 TI가 요구치 아래로 떨어진 뒤 `grade_recalc` | 새 스냅샷도 L1(J8), TI는 떨어진 값으로 기록 |
...
- **01**: §2.2(JUDGING 설정됨), §3.3(집행·반환·리필 방식, 관측 마감 후 제보 불가), §4.1·4.3·4.4·5.1~5.3의 미구현 표기, §6 note(공식 등급 = 주간 스냅샷 J5, 강등 규칙 전까지 등급 유지 J8).
...
- docker compose 수동 확인: 빈 DB에서 V28 적용 후 정상 기동, 옛 판정 행이 남은 DB에서는 V28이 안내와 함께 기동을 멈춘다. (제보 → 판정 → 등급 재계산 한 바퀴는 `verdict_runner`에 수동 실행 수단이 없어 통합 테스트 §7.2 1·15번이 대신한다.)
...
- **J8은 임시 규칙이다** — Phase 2에서 01 §6.1 강등 규칙(90일 롤링 미달 4주 연속, 1단계씩)이 들어오면 그 규칙으로 대체한다.
