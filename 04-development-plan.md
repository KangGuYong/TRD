...
| 1시간 | `sla_watch` | **구현됨(SP3)**. 신고 4h→`TEMP_HIDDEN`(신고는 `OPEN` 유지, `reports.auto_hidden_at` 기록) / 병합 대기 24h→두 항목 마감을 `max(현재 마감, now+24h)`(상한 최초 제보+21일)로 연장, `JUDGING`이고 새 마감이 미래면 `PENDING`으로 복귀 / 90일 미접속 관리자 비활성화(로그인·활성화·재활성화 중 가장 늦은 시각 기준, 마지막 활성 ADMIN은 건너뜀). 유예 연장은 `trend_items.judgment_deadline_override`(수동 `VerdictAdminService.extendGrace`와 같은 컬럼)에 반영되며 `grace_until` 컬럼은 여전히 없다. 세 단계(신고·병합·계정)는 서로 독립이고 단계 안에서도 행 하나가 한 트랜잭션(잠근 뒤 조건 재확인) — 한 행이 실패해도 나머지는 돈다. 통보는 여전히 `SlaNotifier`(Phase 1 로그, 웹훅은 Phase 2·O10) |
